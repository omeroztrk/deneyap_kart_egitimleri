...
void setup(){
  pinMode(led, OUTPUT); //Çıkış pinleri tanımlanmıştır.
  Serial.begin(115200); //Serial haberleşme başlatılmıştır.
  irrecv.enableIRIn();
} //Bu komut ile kızılötesi bilgi alımı başlatılmıştır.
void loop() {
  if (irrecv.decode(&results)) //Bu komut ile veri gelip gelmediği kontrol edilmiştir. Gelen veri hangi butonsa hangi işlemi yapacağı belirtilmiş ve ekrana yazdırılmıştır. Bu şekilde LED yanma ve sönme işlemi gerçekleştirilmiştir.
  {
